...
function extractToolSteps(messages: DeriveInput['messages']): BrunoProgressStep[] {
  const message = lastAssistantMessage(messages);
...
export function deriveBrunoProgressState(input: DeriveInput): BrunoProgressState {
  const isStreaming =
    input.chatStatus === 'streaming' || input.chatStatus === 'submitted';
  const isBrunoWorking = isStreaming;
  ...
  if (serverProgress) {
    ...
    progressSummary = fallback.summary;
