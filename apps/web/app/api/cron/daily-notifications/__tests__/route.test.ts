...
vi.mock('@/lib/notifications/weekly-review', () => ({
  runWeeklyReviewSweep: vi.fn().mockResolvedValue({ sent: 0, failed: 0, total: 0 }),
}));
...
describe('daily notifications cron', () => {
  ...
  it('rejects requests without CRON_SECRET authorization', async () => {
    ...
    const req = new NextRequest('http://localhost:3000/api/cron/daily-notifications');
