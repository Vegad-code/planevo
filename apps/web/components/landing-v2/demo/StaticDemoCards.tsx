        ...
        {LANDING_TASKS.map((task) => (
          ...
          >
            {task.done ? (
              ...
              <Circle size={18} className="flex-none text-[var(--color-ink-soft)]" />
            )}
            <span
              className={`min-w-0 flex-1 text-[14px] ${task.done ? 'text-[var(--color-ink-soft)] line-through' : 'text-[var(--color-ink)]'}`}
            >
              {task.title}
            </span>
            <span className="flex-none font-mono text-[10px] uppercase tracking-wider text-[var(--color-ink-soft)]">
              {task.due}
            </span>
          </li>
        ))}
