...
export default function EventDialog({ isOpen, onOpenChange, event, onSave, onDelete }: EventDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="p-0 overflow-hidden w-[95vw] sm:max-w-[520px] bg-background border-border shadow-2xl rounded-[24px] sm:rounded-[32px] max-h-[90vh] flex flex-col">
        ...
        >
          ...
          <div className="flex gap-4 sm:gap-6 items-end">
            ...
            <div className="relative shrink-0 hidden xs:block">
              <div className="w-16 h-28 sm:w-24 sm:h-40 bg-black/20 backdrop-blur-md border border-white/20 rounded-full flex flex-col items-center justify-center gap-2 sm:gap-4 relative overflow-hidden">
                ...
                <div className="w-10 h-16 sm:w-14 sm:h-24 bg-white/10 rounded-full border border-white/20 flex items-center justify-center relative shadow-inner">
                   ...
                   <div className="absolute -bottom-1 -left-1 sm:-bottom-2 sm:-left-2 w-6 h-6 sm:w-8 sm:h-8 bg-black/40 border border-white/20 rounded-full flex items-center justify-center shadow-lg">
              ...
              <div className="flex items-center gap-2 sm:gap-3 mt-3 sm:mt-4">
                ...
                <button 
                  ...
                  className={cn(
                    ...
                    isCompleted 
                      ? "bg-white border-white text-brand-600 scale-110" 
        ...
        <div className="p-4 sm:p-6 space-y-3 sm:space-y-4 overflow-y-auto custom-scrollbar flex-1 min-h-0">
          {/* Date Info */}
          <div className="bg-muted/50 dark:bg-card/50 rounded-xl sm:rounded-2xl p-3 sm:p-4 flex items-center justify-between border border-border/50">
            <div className="flex items-center gap-3">
              ...
              <div className="font-bold text-foreground/90 text-sm sm:text-base">
            ...
            <Checkbox 
              ...
              className="border-muted-foreground/30 data-[state=checked]:bg-brand-500 data-[state=checked]:text-white"
            />
          </div>
              ...
              {subtasks.map((task, i) => (
                <div key={i} className="flex items-center gap-3 group">
                  <Checkbox 
                    ...
                    className="border-muted-foreground/30 rounded-md data-[state=checked]:bg-brand-500 data-[state=checked]:border-brand-500 h-4 w-4 sm:h-5 sm:w-5"
                  />
                  <span className={cn(
                    "text-xs sm:text-sm font-medium transition-all",
                    task.completed ? "text-muted-foreground/40 line-through" : "text-foreground/80"
                  )}>
                    {task.title}
                  </span>
                </div>
          ...
          {/* Notes Section */}
          <div className="bg-muted/50 dark:bg-card/50 rounded-xl sm:rounded-2xl overflow-hidden border border-border/50">
             <div className="p-3 sm:p-4 flex items-center gap-3 border-b border-border/50 text-muted-foreground/40">
             ...
             <Textarea 
               ...
               className="bg-transparent border-none min-h-[100px] sm:min-h-[120px] focus-visible:ring-0 p-3 sm:p-4 text-foreground font-medium placeholder:text-muted-foreground/40 resize-none text-sm sm:text-base leading-relaxed"
             />
          </div>
        </div>

        {/* Footer */}
        <div className="p-4 sm:p-6 bg-background border-t border-border flex items-center justify-between shrink-0">
          <Button
            variant="ghost"
            className="text-muted-foreground/40 hover:text-red-500 hover:bg-red-500/10 rounded-full h-10 sm:h-12 px-4 sm:px-6"
            onClick={() => {
              onDelete(event.id);
              onOpenChange(false);
            }}
