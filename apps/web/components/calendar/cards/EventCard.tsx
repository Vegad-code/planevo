...
interface EventCardProps {
  ...
  onDragEnd?: (eventId: string, deltaX: number, deltaY: number) => void;
...
export default function EventCard({
  ...
  const handleDragEnd = useCallback(
    (_e: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
      setIsDragging(false);
      const snappedDeltaY = snap(info.offset.y);
      const deltaX = info.offset.x;
      
      if ((Math.abs(snappedDeltaY) >= SNAP_PX || Math.abs(deltaX) > 20) && onDragEnd) {
        onDragEnd(event.id, deltaX, snappedDeltaY);
      }
      dragX.set(0);
      dragY.set(0);
    },
    [event.id, onDragEnd, dragX, dragY]
  );
