...
export function MemoryControls({ initialData }: { initialData: UserAiMemory }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-6 space-y-6 mt-6 border-t border-settings-border pt-6">
                ...
                return (
                  <div key={type} className="bg-settings-card border border-settings-border rounded-xl overflow-hidden">
                    <button 
                      ...
                      className="w-full p-4 flex items-center justify-between hover:bg-(--color-ink)/5 dark:hover:bg-settings-brand/10 transition-colors"
                    >
                      <div className="flex items-center gap-3">
                        <span className="font-bold text-sm text-settings-text">{categoryLabels[type]}</span>
                        <span className="text-xs bg-(--color-ink)/5 dark:bg-settings-brand/10 text-settings-text-muted px-2 py-0.5 rounded-full">{groupItems.length}</span>
                      </div>
                          ...
                          return (
                            <div key={item.id} className="flex flex-col">
                              <button 
                                ...
                                className={`p-4 flex flex-col gap-1 text-left transition-colors ${!isEditing ? 'hover:bg-(--color-ink)/5 dark:hover:bg-settings-brand/10 cursor-pointer' : ''}`}
                              >
                                <div className="flex items-center justify-between w-full">
                                  <div className="flex items-center gap-2">
                                    <span className="text-[10px] font-bold uppercase tracking-wider bg-(--color-ink)/10 text-(--color-ink) dark:bg-settings-brand/20 dark:text-settings-brand px-2 py-0.5 rounded-full">
                                      {item.typeLabel}
                                    </span>
                              ...
                              {isExpanded && !isEditing && (
                                <div className="px-4 pb-4 pt-1 bg-(--color-ink)/5 dark:bg-settings-brand/5 border-t border-settings-border/50 animate-fade-in">
                                  ...
                                  <div className="space-y-3">
                                    ...
                                    {(item.type === 'disliked_patterns' || item.type === 'accepted_patterns' || item.type === 'task_duration_preferences' || item.type === 'task_time_preferences') && (
                                      <div>
                                        ...
                                        <textarea
                                          ...
                                          className="w-full bg-transparent border border-settings-border rounded-lg p-2 text-sm text-settings-text focus:border-settings-text outline-none transition-colors resize-none min-h-15"
                                        />
                                      </div>
                                    )}
                                    <div className="flex gap-2 pt-2">
                                      <button
                                        ...
                                        disabled={saving}
                                        className="px-4 py-1.5 bg-(--color-ink) text-(--color-paper) dark:bg-settings-brand dark:text-(--color-paper) rounded-lg text-xs font-bold uppercase tracking-widest disabled:opacity-50"
                                      >
                                        {saving ? 'Saving...' : 'Save'}
                                      </button>
                                      <button
          ...
          <Dialog open={isResetModalOpen} onOpenChange={setIsResetModalOpen}>
            ...
            <DialogContent className="sm:max-w-md bg-settings-card border-settings-border text-settings-text">
              ...
              <DialogFooter className="mt-4 flex sm:justify-between gap-2">
                <DialogClose asChild>
                  <button className="flex-1 px-4 py-2 bg-transparent border border-settings-border text-settings-text rounded-lg text-xs font-bold uppercase tracking-widest hover:bg-(--color-ink)/5 dark:hover:bg-settings-brand/10 transition-colors">
                    Cancel
                  </button>
                </DialogClose>
                <button 
