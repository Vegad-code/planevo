...
import { BRUNO_PROGRESS_STEP_IDS } from './bruno-progress';
...
/** Index to loop back to after the sequence ends (cooking, always index 1). */
...
export function isBrunoFinalizingPhase(input: FinalizingPhaseInput): boolean {
  const generateActive = input.progressSteps.some(
    (step) =>
      step.id === BRUNO_PROGRESS_STEP_IDS.generate && step.status === 'active'
  );
