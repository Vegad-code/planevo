      ...
      <AnimatePresence mode="wait">
        {view === 'sync' ? (
              ...
              <Card className="bg-white border-4 border-surface-900 rounded-[2rem] shadow-md overflow-hidden">
                ...
                <CardContent className="p-0 max-h-60 overflow-y-auto no-scrollbar">
                  {assignments.length === 0 ? (
                    ...
                    <ul className="divide-y divide-surface-100">
                      {assignments.map((a) => (
                        <li key={a.id} className="p-4 text-xs font-bold text-surface-700">{a.name}</li>
                      ))}
                    </ul>
                  )}
                </CardContent>
          ...
          >
            ...
            {/* Ollie Sidebar */}
            {showOllie && (
              <div className="lg:col-span-4 sticky top-8 animate-in slide-in-from-right duration-500">
                <OllieChatSidebar 
